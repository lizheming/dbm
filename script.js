...
$('body').on('click', '.poster-wall', function() {
    function posterWall(DOM) {
        DOM = DOM || $("#msh")[0];
...
function exportData(m, y, w) {
    m = m.filter(function(item) {return item[6] == y});
    var RATE = new Array(), 
        MONTH = new Array(), 
        DAY = new Array(), 
        TOTAL = m.length, 
    ...
    $.each(m, function(k, item){
        ...
        LI += '<img src="'+item[2]+'" crossOrigin="*" alt="'+item[1]+'" width="67px" height="97px" />';
        LI += '</a></li>';
    });
    ...
    $('#command').html('<span class="1">一星</span><span class="2">二星</span><span class="3">三星</span><span class="4">四星</span><span class="5">五星</span><span class="0">所有</span><button class="poster-wall" style="float: right;">生成海报墙</button>');
    $('#command').css({
...
$('#wrapper').on('change', '#year', function(e) {
    ...
    var m = localStorage[u] ? JSON.parse( localStorage[u] ) : [];
    if( m.length != n/1 ) m = [];
    else return exportData(m, y, w);
    	...
    	$.ajax({
    		...
    		success: function(d) {
    			...
    			$.each($('.item', d), function(j, item){
					...
					m.push([id, title, img, date, rate, des, year]);
    			});	
    			/* recursion */
    			if( i<(n-15) ) {
    				collect(i+15);
